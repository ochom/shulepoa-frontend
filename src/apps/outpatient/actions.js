...
export const updateInvestigation = (id, data) => (dispatch, getState) => {
  dispatch({ type: commonTypes.PROCESSING });
  Axios.patch(`${API_PATH}outpatient/investigations/${id}/`, JSON.stringify(data), tokenConfig(getState))
    .then(() => {
      dispatch(getInvestigations())
...
export const updatePrescription = (id, data) => (dispatch, getState) => {
  dispatch({ type: commonTypes.PROCESSING });
  Axios.patch(`${API_PATH}outpatient/prescriptions/${id}/`, JSON.stringify(data), tokenConfig(getState))
    .then(() => {
      dispatch(getPrescriptions())
